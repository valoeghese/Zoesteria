...
public class WorldGenEvilTreeSapling extends WorldGenAbstractTree
{
	...
	private void generateBranch(World worldIn, BlockPos origin, Random rand)
	{
		...
		for (int distance = 1; distance <= branchLength; ++distance)
		{
			...
			newPos = new BlockPos(0, 0, 0);
			newPos = origin.add(xOffset * distance, yOffset * distance, zOffset * distance);
			if (worldIn.getBlockState(newPos).getBlock() == Blocks.AIR) this.setBlockAndNotifyAdequately(worldIn, newPos, TRUNK.withProperty(BlockLog.LOG_AXIS, d1.getLogAxisOf()));
		}
	...
	private void generateLeafNode(World worldIn, BlockPos pos, Random rand)
	{
		...
		if (worldIn.getBlockState(pos.up()).getBlock() == Blocks.AIR) this.setBlockAndNotifyAdequately(worldIn, pos.up(), LEAVES);
		if (worldIn.getBlockState(pos.add(-1, 1, 0)).getBlock() == Blocks.AIR) this.setBlockAndNotifyAdequately(worldIn, pos.add(-1, 1, 0), LEAVES);
		if (worldIn.getBlockState(pos.add(1, 1, 0)).getBlock() == Blocks.AIR) this.setBlockAndNotifyAdequately(worldIn, pos.add(1, 1, 0), LEAVES);
		if (worldIn.getBlockState(pos.add(0, 1, -1)).getBlock() == Blocks.AIR) this.setBlockAndNotifyAdequately(worldIn, pos.add(0, 1, -1), LEAVES);
		if (worldIn.getBlockState(pos.add(0, 1, 1)).getBlock() == Blocks.AIR) this.setBlockAndNotifyAdequately(worldIn, pos.add(0, 1, 1), LEAVES);
			...
			for (int x = -2; x < 3; ++x)
			{
				for (int z = -1; z < 2; ++z)
				{
					if (x != 0 || z != 0)
					{
						...
						if (worldIn.getBlockState(newPos).getBlock() == Blocks.AIR) this.setBlockAndNotifyAdequately(worldIn, pos.add(x, y, z), LEAVES);
					}
				}

				if (Math.abs(x) < 2)
				{
					if (worldIn.getBlockState(pos.add(x, y, -2)).getBlock() == Blocks.AIR) this.setBlockAndNotifyAdequately(worldIn, pos.add(x, y, -2), LEAVES);
					if (worldIn.getBlockState(pos.add(x, y, 2)).getBlock() == Blocks.AIR) this.setBlockAndNotifyAdequately(worldIn, pos.add(x, y, 2), LEAVES);
				}
			}
		}
	...
	protected void setBlockAndNotifyAdequately(World worldIn, BlockPos pos, IBlockState stateIn)
	{
        ...
        if (state.getBlock() == Blocks.AIR || state.getMaterial() == Material.PLANTS || state.getMaterial() == Material.VINE)
        {
