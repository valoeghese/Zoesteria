...
public class BiomeLushJungle extends Biome
{
	...
	public BiomeLushJungle()
	{
		super(new Biome.BiomeProperties("Tropical Jungle").setBaseHeight(Biomes.JUNGLE.getBaseHeight()).setHeightVariation(Biomes.JUNGLE.getHeightVariation()).setRainfall(0.95F).setTemperature(Biomes.JUNGLE.getDefaultTemperature()));
	...
	{
		if ((noiseVal > 2D && noiseVal < 2.9D) || (noiseVal < -2D && noiseVal > -2.43D) )
		{
			if (noiseVal > 2.23D && noiseVal < 2.67D)
			{
				if (noiseVal > 2.37D && noiseVal < 2.53D)
				{
					this.topBlock = Blocks.STAINED_HARDENED_CLAY.getDefaultState().withProperty(BlockColored.COLOR, EnumDyeColor.YELLOW);
				} else {
					this.topBlock = Blocks.STAINED_HARDENED_CLAY.getDefaultState().withProperty(BlockColored.COLOR, EnumDyeColor.ORANGE);
				}
			} else {
				this.topBlock = Blocks.STAINED_HARDENED_CLAY.getDefaultState().withProperty(BlockColored.COLOR, EnumDyeColor.RED);
			}
		} 
		else {
