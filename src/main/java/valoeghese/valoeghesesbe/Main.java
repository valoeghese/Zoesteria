...
public class Main
{
	...
	public static boolean firstLoadBoolean = true;
	...
	public static BushGen genStandard = new BushGen(ModBlocks.SMALL_BUSH.getDefaultState(), Type.FOREST, Type.LUSH).removeBiomeFromSet(ModBiomes.VBE_FEN).removeBiomeFromSet(ModBiomes.VBE_ORCHID_FIELD);
	...
	{
		/*
    	if(Loader.isModLoaded("Exotic Birds"))
    	{
    		System.out.println("Zoesteria has detected you have Vivid Birds installed. Loading compatability...");
    		Reference.isVividBirdsInstalled = true;
    		
    		new CompatBirds().adaptMod();
    	}
    	
    	*/
		
		if (newGenerationBoolean) MinecraftForge.TERRAIN_GEN_BUS.register(TerrainHandler.class);
    	
		ModBiomes.RegisterBiomes();
		
		for (BushGen bush : bushes)
		{
